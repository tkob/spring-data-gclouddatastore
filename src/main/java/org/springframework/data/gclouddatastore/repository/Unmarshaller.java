...
public class Unmarshaller {
        ...
        switch (valueType) {
            ...
            case RAW_VALUE:
                throw new UnsupportedOperationException(
                    valueType.toString());
            case TIMESTAMP:
                return ((TimestampValue)value).get().toSqlTimestamp()
                    .toInstant();
            default:
    ...
    public <K extends IncompleteKey> void unmarshalToMap(
            ...
            switch (valueType) {
                case ENTITY:
                    if (map.containsKey(name)) {
                        unmarshalToObject(entity.getEntity(name),
                            map.get(name));
                    }
                    else {
                        Map<String, Object> newMap = new HashMap<>();
    ...
    public <K extends IncompleteKey> void unmarshalToObject(
        FullEntity<K> entity, Object object) {
        ...
        BeanWrapper beanWrapper =
            PropertyAccessorFactory.forBeanPropertyAccess(object);
        for (String name : entity.getNames()) {
            Value<?> value = entity.getValue(name);
            ValueType valueType = value.getType();
            ...
            switch (valueType) {
                ...
                case ENTITY:
                    if (targetType.isAssignableFrom(Map.class)) {
                        beanWrapper.setPropertyValue(
                            name, unmarshal(value, Map.class));
                    }
                    else if (Map.class.isAssignableFrom(targetType)) {
                        @SuppressWarnings("unchecked")
                        Map<String, Object> map =
                            (Map<String, Object>)beanWrapper
                                .getPropertyValue(name);
                        if (map == null) {
                            try {
                                map = (Map<String, Object>)targetType
                        ...
                        if (targetObject == null) {
                            try {
                                targetObject = targetType.getConstructor()
                                    .newInstance();
                            } catch (ReflectiveOperationException e) {
                                break;
                            }
                        ...
                        unmarshalToObject(
                            ((EntityValue)value).get(), targetObject);
                    }
                    break;
                case KEY:
                    break;
                ...
                case LIST:
                    if (targetType.isAssignableFrom(List.class)) {
                        beanWrapper.setPropertyValue(
                            name,
                            unmarshal(value, List.class));
                    }
                    else if (List.class.isAssignableFrom(targetType)) {
                        @SuppressWarnings("unchecked")
                        List<Object> newList =
                            (List<Object>)beanWrapper.getPropertyValue(name);
                        if (newList == null) {
                            try {
                                newList = (List<Object>)targetType
                                    .getConstructor().newInstance();
                            } catch (ReflectiveOperationException e) {
                                break;
                            }
                ...
                case TIMESTAMP:
                    if (targetType.isAssignableFrom(Instant.class)) {
                        beanWrapper.setPropertyValue(
                            name, unmarshal(value, Instant.class));
                    }
